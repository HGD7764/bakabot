...
module.exports = (context) => {
      ...
      switch (args.length) {
        ...
        case 2:
          ...
          if (isNaN(x_xz) || isNaN(z_xz)) {
            bot.whisper(username, '> 参数错误：坐标格式无效。'); return;
          }
          targetGoal = new GoalNear(x_xz, bot.entity.position.y, z_xz, 1);
          bot.whisper(username,`> 指令已接收。目标坐标：X:${x_xz.toFixed(0)}, Z:${z_xz.toFixed(0)}。`);
          break;
        ...
        case 3:
          ...
          bot.whisper(username,`> 指令已接收。目标坐标：X:${x_xyz.toFixed(0)}, Y:${y_xyz.toFixed(0)}, Z:${z_xyz.toFixed(0)}。`);
          break;

        default:
          bot.whisper(username, '> 参数错误：无效参数数量。');
          return;
      }
  ...
  commands.register({
    ...
    execute: (username, args) => {
      ...
      bot.whisper(username,`> 指令已接收。开始跟随：${targetPlayerName}。`);
    },
  });
