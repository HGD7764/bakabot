...
module.exports = (context) => {
  ...
  commands.register({
    ...
    execute: async (username, args) => {
      if (args.length === 0) {
        bot.whisper(username, '> 参数错误：需要提供方块名或坐标。');
        return;
      }
      ...
      if (!isNaN(parseFloat(args[0])) && args.length >= 3) {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(z)) {
          bot.whisper(username, '> 参数错误：坐标格式无效，请输入三个数字。');
          return;
        }
        
        targetPosition = vec3(x, y, z); // <--- 修改：不再使用 bot.vec3
        bot.whisper(username,`> 指令已接收。正在转向坐标: ${targetPosition.toString()}`);
        ...
        if (!blockType) {
          bot.whisper(username,`> 目标错误：未知的方块名称 '${blockName}'。`);
          return;
        }
        ...
        if (!block) {
          bot.whisper(username,`> 目标错误：在附近找不到 '${blockName}'。`);
          return;
        }
        
        targetPosition = block.position.add(vec3(0.5, 0.5, 0.5));
        bot.whisper(username,`> 指令已接收。正在转向最近的 ${blockName}。`);
      }

      try {
        await bot.lookAt(targetPosition);
        bot.whisper(username, '> 任务完成：已面向目标。');
      } catch (err) {
        bot.whisper(username, '> 系统异常：转向时发生错误。');
  ...
  commands.register({
    ...
    execute: async (username, args) => {
      if (args.length === 0) {
        bot.whisper(username, '> 参数错误：需要指定 left 或 right。');
        return;
      }
      
      const targetBlock = bot.blockAtCursor(5); // 获取光标指向的5格内的方块
      if (!targetBlock) {
        bot.whisper(username, '> 目标错误：未面向任何方块。');
        return;
      }
      
      bot.whisper(username,`> 指令已接收：正在与 ${targetBlock.name} 交互...`);
      ...
      try {
        if (handSide === 'left') {
          ...
          bot.whisper(username, '> 参数错误：只能是 left 或 right。');
        }
      } catch (err) {
         bot.whisper(username,`> 系统异常：与方块交互时发生错误。`);
  ...
  commands.register({
    ...
    execute: (username, args) => {
      if (args.length === 0) {
        bot.whisper(username, '> 参数错误。用法: !loopclick <left|right> [间隔] 或 !loopclick stop');
        return;
      }
      ...
      if (action === 'stop') {
        if (loopTask) {
          ...
          loopTask = null;
          bot.whisper(username, '> 循环任务已中断。');
        } else {
          bot.whisper(username, '> 无正在执行的循环任务。');
        }
        return;
      }
      ...
      if (isNaN(interval) || interval < 50) {
        bot.whisper(username, '> 参数错误：间隔时间必须是大于50的数字。');
        return;
      }

      bot.whisper(username,`> 循环任务已启动：${action} 点击，间隔 ${interval}ms。使用 !loopclick stop 停止。`);
